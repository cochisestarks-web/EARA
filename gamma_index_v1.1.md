...
### 2.2 Continuous Differential Form (Locked, April 2026)

The discrete accumulation formula (Section 2.1) is implemented as a summation over hourly steps. The equivalent continuous formulation is:

$$\frac{d\gamma}{dt} = \lambda \cdot C(t) \cdot W(t) - \mu \cdot (1 - W(t)) \cdot \gamma(t)$$

**Where:**
- `λ = 0.0097` — accumulation coefficient (Darwish 2023; allostatic load kinetics)
- `μ = 0.0009` — recovery coefficient (Borbély two-process model, Process S decay)
- `W(t)` — work state function: W(t) = 1 if working, 0 if resting
- `C(t)` — circadian modulation scalar: `C(t) = 1 + α · cos(2π(t − t_peak)/24)`
- `α = 0.15` — circadian amplitude (provisional; typical 10–20% modulation range in literature)
- `t_peak = 15:00` — circadian vulnerability peak (afternoon trough, post-lunch dip)

**Boundary conditions:**
- γ(0) = 0 at full rest baseline
- γ(t) ∈ [0, 1.0]

**Recovery term note:** The recovery term includes γ(t) — recovery rate slows at high debt levels, consistent with allostatic load saturation. Flat μ without γ(t) would ignore physiological limits on recovery speed.

**Discrete vs. Continuous Reconciliation:**
The discrete formulation (Section 2.1) caps C_penalty at a 6-hour night shift ceiling as a practical constraint to prevent runaway accumulation from extreme schedules. In the continuous ODE, this constraint is implicit in the bounded cosine modulation (α = 0.15, oscillating between 0.85–1.15), which prevents runaway accumulation without requiring an explicit cap. Numerical validation (April 2026) confirms both versions diverge by less than 3% across tested schedules (8-hour night shift: 2.17%; 12-hour night shift: 0.04%). The two formulations are functionally consistent.
...
**v1.1 (April 9, 2026):**
- Continuous differential form of γ(t) added (Section 2.2) — locked equation with sourced parameters
- Discrete vs. continuous reconciliation documented and numerically validated (divergence < 3%)
- α = 0.15 flagged explicitly as provisional pending sensitivity analysis
- Recovery term γ(t) dependency confirmed as physiologically required
